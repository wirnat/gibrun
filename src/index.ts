...
import { GoDebuggerProxy } from "./goDebuggerProxy.js";
...
async function handlePostgresQuery(args: any) {
    ...
    try {
        const connectionString = resolveConnectionString(connection_string);
        ...
        logError("postgres_query failed", error, { tool: "postgres_query" });
        return {
            content: [
                {
                    type: "text",
...
async function handleHttpRequest(args: any) {
    ...
    try {
        ...
        if (error.response) {
            ...
            logError("http_request returned error response", error, {
                tool: "http_request",
                url,
                method,
            ...
            logError("http_request received no response", error, {
                tool: "http_request",
                url,
                method,
                reason: "no_response",
            ...
            // Request setup error
            logError("http_request failed to initialize request", error, {
                tool: "http_request",
                url,
                method,
                reason: "request_setup",
            });
            return {
                content: [
                    {
