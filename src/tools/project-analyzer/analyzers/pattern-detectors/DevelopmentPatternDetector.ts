...
export class DevelopmentPatternDetector {
  static detectTDDPattern(data: RawProjectData): { detected: boolean; confidence: number; evidence: string } {
    ...
    // Look for test files
    const testFiles = files.filter(f => f.path?.toLowerCase().includes('test') || f.path?.toLowerCase().includes('spec'));
    const testRatio = testFiles.length / Math.max(files.length, 1);

    // Look for test-driven commit patterns
    const testCommits = gitHistory.filter(commit =>
      commit.message?.toLowerCase().includes('test') ||
      commit.message?.toLowerCase().includes('spec') ||
      commit.message?.toLowerCase().includes('tdd')
    );

    const testCommitRatio = testCommits.length / Math.max(gitHistory.length, 1);

    const indicators = testRatio + testCommitRatio;
    const confidence = Math.min(1.0, indicators);

    return {
      detected: indicators >= 0.3,
      confidence,
      evidence: `${testFiles.length} test files (${(testRatio * 100).toFixed(1)}%), ${testCommits.length} test-related commits`
    };
  }
  ...
  static analyzeDevelopmentPatterns(data: RawProjectData): IdentifiedPattern[] {
    ...
    if (tdd.detected) {
      patterns.push({
        ...
        implications: 'High code quality, reliable refactoring, comprehensive test coverage',
